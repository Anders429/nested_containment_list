...
trait NestedContainment<R, T> where R: RangeBounds<T> {
    fn contains<S>(&self, inner: &S) -> bool where S: RangeBounds<T>;
}

impl<R, T> NestedContainment<R, T> for R where R: RangeBounds<T>, T: PartialOrd<T> {
